...
class deepHunt:
	...
	def build(width, height, depth, classes, init_mode, actFunc): 
		...
		model.add(Dense(500,kernel_initializer=init_mode))
		model.add(Activation(actFunc))

		# softmax classifier
		model.add(Dense(classes,kernel_initializer=init_mode)) # the number of nodes is equal to the number of classes , which is 2 in my case 
