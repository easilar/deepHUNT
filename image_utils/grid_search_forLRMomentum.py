...
def create_model(lr=0.01, momentum=0.9, optimizer='Adam', width=conf.sizeX, height=conf.sizeY):
	#get model
...
for n,optimizer in enumerate(optimizers):
	for lr in lrs:
		for momentum in momentums:
			model_save_name = '_'.join(['optAlg'+Noptimizers[n],'lr'+str(lr).replace('.','p'),'mom'+str(momentum).replace('.','p')])
			model_dir = "models_gridSearch/"+conf.tag+"no"+conf.tag+str(conf.sizeX)+'_'+model_save_name+"_confGen_multiGPU.h5"
			if not os.path.exists(model_dir):
				model = create_model(optimizer=optimizer,lr=lr, momentum=momentum)
