...
from metric_helper import precision , sensitivity, specificity, accuracy, bal_acc, fpr, fnr, fmeasure, mcc, youden, AUC , gmean , auc_roc
import time

for nFilt in conf.nFilts:
	for kernS in conf.kernSs:
		for denseL in conf.denseLs:
			print(nFilt,kernS,denseL) 
			for INIT_LR in conf.INIT_LRs:
				for EPOCHS in conf.EPOCHSs:

					model_save_name = '_'.join([str(nFilt[0]),str(nFilt[1]),str(kernS[0]),str(denseL),str(INIT_LR),str(EPOCHS)])

					model_s = deepHunt.build(nFilt=nFilt, kernS=kernS, denseL=denseL, width=conf.sizeX, height=conf.sizeY, depth=3, classes=2)
					...
					model.compile(loss="binary_crossentropy", optimizer=opt,
						metrics=["accuracy",mae,categorical_accuracy,auc_roc,'binary_accuracy',precision , sensitivity, specificity, accuracy, bal_acc, fpr, fnr, fmeasure, mcc, youden, gmean])
